...
CREATE TABLE IF NOT EXISTS `dbx-external-catalog`.analytics.daily_revenue_by_type (
  business_date DATE,
  booking_type STRING,
  total_amount DOUBLE,
...
DELETE FROM `dbx-external-catalog`.analytics.daily_revenue_by_type
...
INSERT INTO `dbx-external-catalog`.analytics.daily_revenue_by_type (business_date, booking_type, total_amount, total_quantity)
SELECT
...
FROM `dbx-external-catalog`.bronze.booking_inc
WHERE business_date = COALESCE(TRY_CAST(:arrival_date AS DATE), current_date())
