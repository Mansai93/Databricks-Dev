...
CREATE SCHEMA IF NOT EXISTS `dbx-external-catalog`.analytics;
CREATE SCHEMA IF NOT EXISTS `dbx-external-catalog`.ops;
...
CREATE TABLE IF NOT EXISTS `dbx-external-catalog`.ops.workflow2_run_log (
   run_id STRING,
   arrival_date DATE,
   status STRING,
   message STRING,
