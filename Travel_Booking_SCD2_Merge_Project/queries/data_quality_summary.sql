...
CREATE TABLE IF NOT EXISTS `dbx-external-catalog`.ops.dq_daily_summary (
  business_date DATE,
  dataset STRING,
  checks_passed INT,
  checks_failed INT,
...
MERGE INTO `dbx-external-catalog`.ops.dq_daily_summary t
USING (
  SELECT
    COALESCE(TRY_CAST(:arrival_date AS DATE), current_date()) AS business_date,
    dataset,
    SUM(CASE WHEN status = 'Success' THEN 1 ELSE 0 END) AS checks_passed,
    SUM(CASE WHEN status <> 'Success' THEN 1 ELSE 0 END) AS checks_failed,
    current_timestamp() AS recorded_at
  FROM `dbx-external-catalog`.ops.dq_results
